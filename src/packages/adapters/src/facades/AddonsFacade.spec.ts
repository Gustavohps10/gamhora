...
`
    ...
    if (result.isSuccess()) {
      expect(result.success.id).toBe('pandhora-datasource-redmine')
...
`
    ...
    expect(result.isSuccess()).toBe(true)
    if (result.isSuccess()) {
      expect(result.success.id).toBe('pandhora-legacy')
  ...
  it('should fetch available addons in a single HTTP request without N+1', async () => {
    ...
    if (result.isSuccess()) {
      ...
      expect(result.success[0].id).toBe('pandhora-datasource-redmine')
      expect(result.success[0].downloadUrl).toBe(
        'https://example.com/redmine.tladdon',
      )
      expect(result.success[1].id).toBe('pandhora-watcher-discord')
    }
  })
})
