...
O **Mr-tick** utiliza um motor de sincronização agnóstico baseado em **HTTP Replication (Pull & Push de Deltas)**. Em vez de abrir conexões contínuas em tempo real (como WebSockets persistentes) ou fazer polling agressivo contra APIs de terceiros, o Mr-tick sincroniza dados de forma estruturada, eficiente e resiliente a falhas de rede.

---

## 🔄 Como Funciona a Replicação HTTP

A sincronização entre o Mr-tick e qualquer servidor ou ferramenta de gestão (Redmine, Jira, YouTrack, etc.) é dividida em dois ciclos bem definidos: **Fase de Pull** e **Fase de Push**.
...
- O Mr-tick envia um parâmetro `checkpoint` (que pode ser um timestamp ISO 8601 ou cursor).
...
- Quando o lote é processado com sucesso, o Mr-tick armazena o novo `checkpoint` localmente.
...
2. **Tratamento de Exclusão (`_deleted: true`)**:
   - Se o apontamento foi excluído no Mr-tick local, ele chama `timeEntryRepository.delete(id)` no servidor remoto e marca como sincronizado (`syncedAt: new Date()`).
3. **Detecção de Conflitos (Optimistic Concurrency Control)**:
   - Se o registro já existe no servidor remoto, o Mr-tick compara o `updatedAt` do servidor com o `assumedMasterState.updatedAt` enviado pelo cliente local.
...
No Mr-tick, todo o contexto de dados pertence a um **Workspace** (espaço de trabalho).
...
A camada `@mr-tick/domain` garante a consistência absoluta dos dados de tempo:
...
- **Tarefa de Contingência (`createFallback`)**: Quando o usuário inicia o cronômetro sem selecionar uma tarefa previamente, o Mr-tick cria automaticamente uma _General Task_ de contingência para que nenhum segundo trabalhado seja perdido.
