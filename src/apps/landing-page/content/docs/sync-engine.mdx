...
- O Gamhora envia um parâmetro `checkpoint` (que pode ser um timestamp ISO 8601 ou cursor).
...
- Quando o lote é processado com sucesso, o Gamhora armazena o novo `checkpoint` localmente.
...
2. **Tratamento de Exclusão (`_deleted: true`)**:
   - Se o apontamento foi excluído no Gamhora local, ele chama `timeEntryRepository.delete(id)` no servidor remoto e marca como sincronizado (`syncedAt: new Date()`).
3. **Detecção de Conflitos (Optimistic Concurrency Control)**:
   - Se o registro já existe no servidor remoto, o Gamhora compara o `updatedAt` do servidor com o `assumedMasterState.updatedAt` enviado pelo cliente local.
...
No Gamhora, todo o contexto de dados pertence a um **Workspace** (espaço de trabalho).

### O Papel do Workspace:

- O Workspace atua como o isolamento de _tenant_ (ex: "Empresa A", "Empresa B", "Consultoria Freelance").
...
A camada `@gamhora-org/domain` garante a consistência absoluta dos dados de tempo:
...
- **Tarefa de Contingência (`createFallback`)**: Quando o usuário inicia o cronômetro sem selecionar uma tarefa previamente, o Gamhora cria automaticamente uma _General Task_ de contingência para que nenhum segundo trabalhado seja perdido.
