...
description: Como autenticar usuários com segurança usando PKCE (RFC 7636), State criptográfico e Deep Linking (gamhora-app://).
---

# Autenticação OAuth 2.0 PKCE

O Gamhora fornece uma API nativa e padronizada de **OAuth 2.0 com PKCE (Proof Key for Code Exchange — RFC 7636)** através de `context.oauth`.
...
3. Redirecionamento Web
   ...
   │ https://gamhoraapp.com.br/oauth/callback?code=AUTH_CODE&state=STATE
   ▼
4. Deep Link do Sistema Operacional
   │ A Landing Page executa o protocolo seguro:
   │ gamhora-app://oauth/callback?code=AUTH_CODE&state=STATE
   ▼
5. Addon (Desktop)
...
2. **Proteção Contra Interceptação de Deep Link**: Mesmo que um aplicativo malicioso no sistema operacional capture o Deep Link contendo o `code` e o `state`, ele **não conseguirá** obter o token de acesso porque não possui o `code_verifier` (que permanece restrito à memória segura do Gamhora).
...
```typescript
import type { IAddon, AddonContext } from '@gamhora-org/sdk'
...
export default class DiscordOAuthAddon implements IAddon {
  ...
  public async handleLogin(context: AddonContext): Promise<void> {
    ...
    const REDIRECT_URI =
      ...
      'http://localhost:3000/oauth/callback' // Em produção: https://gamhoraapp.com.br/oauth/callback
