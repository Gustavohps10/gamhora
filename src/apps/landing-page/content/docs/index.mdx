...
description: Conheça a filosofia, a estrutura em camadas e os princípios de segurança do ecossistema Metric.
---

# Visão Geral e Arquitetura

O **Metric** foi projetado com uma arquitetura **100% descentralizada, desacoplada e agnóstica**. 

O aplicativo principal (*Core Desktop*) não possui regras de negócio vinculadas a ferramentas específicas (como Jira, Redmine, YouTrack, Discord, Google ou Microsoft). Todo o poder de expansão, sincronização de tarefas, monitoramento de atividades e temas visuais é exposto através de extensões padronizadas via **`@metric-org/sdk`**.

---

## 🏛️ Arquitetura em Camadas

O ecossistema do Metric é composto por camadas com responsabilidades estritas e bem delimitadas:
...
│    (@metric-org/domain  &  @metric-org/application)    │
...
### 2. Camada de Domínio & Aplicação (`@metric-org/domain` / `@metric-org/application`)
...
### 4. Ponte de Deep Link (`metric-app://`)
Uma ponte universal para fluxos web (como OAuth 2.0). A Landing Page intermediária recebe o retorno de provedores externos e despacha com segurança os parâmetros de volta ao aplicativo desktop através do protocolo `metric-app://oauth/callback`.
