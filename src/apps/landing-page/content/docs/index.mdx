...
description: Conheça a filosofia, a estrutura em camadas e os princípios de segurança do ecossistema Gamhora.
---

# Visão Geral e Arquitetura

O **Gamhora** foi projetado com uma arquitetura **100% descentralizada, desacoplada e agnóstica**.

O aplicativo principal (_Core Desktop_) não possui regras de negócio vinculadas a ferramentas específicas (como Jira, Redmine, YouTrack, Discord, Google ou Microsoft). Todo o poder de expansão, sincronização de tarefas, monitoramento de atividades e temas visuais é exposto através de extensões padronizadas via **`@gamhora-org/sdk`**.

---

## 🏛️ Arquitetura em Camadas

O ecossistema do Gamhora é composto por camadas com responsabilidades estritas e bem delimitadas:

```text
┌────────────────────────────────────────────────────────┐
│                   Gamhora Core Desktop                  │
│  (Janelas, Timers Nativos, Keytar Vault, AddonLoader)  │
└──────────────────────────┬─────────────────────────────┘
                           │
┌──────────────────────────▼─────────────────────────────┐
│                 Motor de Aplicação & Domínio           │
│    (@gamhora-org/domain  &  @gamhora-org/application)    │
...
### 2. Camada de Domínio & Aplicação (`@gamhora-org/domain` / `@gamhora-org/application`)
...
### 4. Ponte de Deep Link (`gamhora-app://`)

Uma ponte universal para fluxos web (como OAuth 2.0). A Landing Page intermediária recebe o retorno de provedores externos e despacha com segurança os parâmetros de volta ao aplicativo desktop através do protocolo `gamhora-app://oauth/callback`.
