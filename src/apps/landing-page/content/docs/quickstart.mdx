...
description: Guia prático e passo a passo para criar, configurar e testar seu primeiro Addon utilizando o SDK do Gamhora.
---

# Criando seu Primeiro Addon

Neste guia prático, você aprenderá a construir um Addon completo para o Gamhora do zero utilizando **TypeScript** e o pacote oficial **`@gamhora-org/sdk`**.
...
```bash
mkdir gamhora-meu-addon
cd gamhora-meu-addon
npm init -y
npm install @gamhora-org/sdk
...
Todo addon do Gamhora exporta como `default` uma classe que implementa o contrato **`IAddon`**.
...
export default class MeuPrimeiroAddon implements IAddon {
  ...
  public description = 'Exemplo prático de extensão para o Gamhora'
  public author = 'Seu Nome ou Empresa'

  private context?: AddonContext

  // Formulário de preferências renderizado automaticamente na interface do Gamhora
  ...
  // Ciclo de Vida: Executado quando o plugin é ativado no Gamhora
  public async onActivate(context: AddonContext): Promise<void> {
    ...
    if (notificar) {
      await context.notifications.show({
        title: 'Addon Ativado',
        body: `Conectado ao Gamhora com sucesso!`,
      })
    }
...
A propriedade `settingsFields` permite que seu plugin defina formulários de configuração sem escrever uma única linha de interface gráfica (UI). O Gamhora gera e valida os campos nativamente.
...
Ao ser ativado, o Gamhora injeta o objeto `AddonContext` contendo todas as APIs e ferramentas disponíveis:
...
O arquivo compilado resultante pode ser importado diretamente nas configurações de Addons do Gamhora.
