...
Todo addon do Mr-tick exporta como `default` uma classe que implementa o contrato **`IAddon`**.
...
export default class MeuPrimeiroAddon implements IAddon {
  ...
  public description = 'Exemplo prático de extensão para o Mr-tick'
  public author = 'Seu Nome ou Empresa'

  private context?: AddonContext

  // Formulário de preferências renderizado automaticamente na interface do Mr-tick
  ...
  public async onActivate(context: AddonContext): Promise<void> {
    ...
    if (notificar) {
      await context.notifications.show({
        title: 'Addon Ativado',
        body: `Conectado ao Mr-tick com sucesso!`,
      })
    }
...
A propriedade `settingsFields` permite que seu plugin defina formulários de configuração sem escrever uma única linha de interface gráfica (UI). O Mr-tick gera e valida os campos nativamente.
