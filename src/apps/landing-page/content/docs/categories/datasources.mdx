...
Um **DataSource** é o tipo de Addon responsável por integrar o Metric a ferramentas de gerenciamento de tarefas e projetos (como Jira, Redmine, YouTrack, GitHub Issues ou Azure DevOps).
...
export default class ExemploDataSourceAddon implements IAddon {
  ...
  public async getAuthenticatedMember(): Promise<{ id: string; name: string; login: string }> {
  ...
  public async pullTasks(memberId: string, checkpoint?: string, batch: number = 50): Promise<TaskDTO[]> {
    const apiUrl = await this.context?.storage.get('apiUrl')
    const token = await this.context?.storage.get('apiToken')
  ...
  public async pushTimeEntry(entry: SyncTimeEntryDTO): Promise<void> {
    ...
    const endpoint = entry.id ? `${apiUrl}/time_entries/${entry.id}` : `${apiUrl}/time_entries`
