...
Este documento explica o fluxo simples e passo a passo para versionar e publicar pacotes do monorepo Metric no **NPM** usando o **Changesets**.
...
Após fazer alterações no código (ex: no `@metric-org/sdk`), rode na raiz:

```bash
yarn changeset
```

1. **Selecione os pacotes alterados**: Use as setas do teclado e a barra de espaço para marcar (ex: `[x] @metric-org/sdk`) e aperte `Enter`.
...
3. Preencha os campos:
   - **Token name**: `metric-publisher`
   - **Packages and scopes**:
     - **Permissions**: `Read and write`
     - **Select packages**: `All packages` (ou escopo `@metric-org`)
   - **Organizations**:
     - **Permissions**: `Read and write`
